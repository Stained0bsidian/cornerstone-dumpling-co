  ...
  if (lines.length === 0) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        <h1 className="font-heading text-brand-green text-4xl font-semibold">Checkout</h1>
        <p className="font-body text-brand-forest/75 mt-4 text-lg">Your cart is empty. Add something delicious first.</p>
        <Link href="/shop" className="font-body bg-brand-green hover:bg-brand-green-accent text-brand-cream mt-8 inline-flex rounded-full px-8 py-3.5 text-sm font-semibold transition-colors">
          Go to shop
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
      ...
      <p className="font-body text-brand-forest/75 mt-3 max-w-2xl">
        Local pickup or local delivery only—we&apos;ll confirm your order by email or phone.
      </p>

      <div className="mt-12 grid gap-12 lg:grid-cols-5">
        <section className="border-brand-green/10 lg:col-span-3 rounded-3xl border bg-white/70 p-6 shadow-sm sm:p-8">
          <h2 className="font-heading text-brand-forest text-2xl font-semibold">Your details</h2>
          <form className="mt-8 space-y-6" onSubmit={handleSubmit} noValidate>
            {error && (
              <p className="font-body rounded-xl bg-red-50 px-4 py-3 text-sm text-red-800" role="alert">{error}</p>
            )}

            <div className="grid gap-6 sm:grid-cols-2">
              <label className="font-body block text-sm font-medium text-brand-forest">
                Full name
                <input required name="customerName" value={customerName} onChange={(e) => setCustomerName(e.target.value)} autoComplete="name"
              ...
              <label className="font-body block text-sm font-medium text-brand-forest">
                Email
                <input required type="email" name="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email"
                  className="border-brand-green/20 focus:border-brand-green focus:ring-brand-green/30 mt-2 w-full rounded-xl border bg-white px-4 py-3 text-brand-forest shadow-sm focus:outline-none focus:ring-2" />
              </label>
            </div>
            ...
            <label className="font-body block text-sm font-medium text-brand-forest">
              Phone
              <input required type="tel" name="phone" value={phone} onChange={(e) => setPhone(e.target.value)} autoComplete="tel"
                className="border-brand-green/20 focus:border-brand-green focus:ring-brand-green/30 mt-2 w-full rounded-xl border bg-white px-4 py-3 text-brand-forest shadow-sm focus:outline-none focus:ring-2" />
            </label>

            <fieldset>
              <legend className="font-body text-sm font-medium text-brand-forest">Fulfillment</legend>
            ...
            {fulfillment === "delivery" && (
              <label className="font-body block text-sm font-medium text-brand-forest">
                Delivery address
                <textarea name="deliveryAddress" value={deliveryAddress} onChange={(e) => setDeliveryAddress(e.target.value)} rows={3}
                  className="border-brand-green/20 focus:border-brand-green focus:ring-brand-green/30 mt-2 w-full rounded-xl border bg-white px-4 py-3 text-brand-forest shadow-sm focus:outline-none focus:ring-2"
            ...
            <label className="font-body block text-sm font-medium text-brand-forest">
              Preferred date &amp; time
              <input required name="preferredDateTime" value={preferredDateTime} onChange={(e) => setPreferredDateTime(e.target.value)}
                className="border-brand-green/20 focus:border-brand-green focus:ring-brand-green/30 mt-2 w-full rounded-xl border bg-white px-4 py-3 text-brand-forest shadow-sm focus:outline-none focus:ring-2"
            ...
            <label className="font-body block text-sm font-medium text-brand-forest">
              ...
              <textarea name="specialInstructions" value={specialInstructions} onChange={(e) => setSpecialInstructions(e.target.value)} rows={3}
                className="border-brand-green/20 focus:border-brand-green focus:ring-brand-green/30 mt-2 w-full rounded-xl border bg-white px-4 py-3 text-brand-forest shadow-sm focus:outline-none focus:ring-2"
        ...
        <aside className="lg:col-span-2">
          <div className="border-brand-green/10 bg-brand-cream/80 sticky top-28 rounded-3xl border p-6 shadow-sm">
            <h2 className="font-heading text-brand-forest text-xl font-semibold">Order summary</h2>
            <ul className="mt-4 space-y-3">
              {lines.map((l) => (
                <li key={l.product.id} className="font-body flex justify-between gap-4 text-sm text-brand-forest/90">
                  <span>{l.quantity}× {l.product.name}</span>
                  <span className="shrink-0 font-medium">{formatPrice(l.product.priceCents * l.quantity)}</span>
                </li>
              ))}
            </ul>
            ...
            <p className="font-body text-brand-forest/65 mt-3 text-xs leading-relaxed">
              You&apos;ll be taken to Stripe&apos;s secure checkout to complete payment. All major cards accepted.
            </p>
          </div>
        </aside>
      </div>
